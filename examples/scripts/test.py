...
def main() -> None:
    data = mobilised.load_recording(
        ...
        tracked_points={"SU": "LowerBack", "SU_INDIP": ["LowerBack", "LeftFoot", "RightFoot"]}
    )
    acc_data = data.data["SU"].loc[:, [c for c in data.data["SU"].columns if "_ACC" in c]]
    ...
    plot_gait_sequences(acc_data=acc_data, sampling_frequency_Hz=fs, gait_sequences=gsd.gait_sequences_)
    return
