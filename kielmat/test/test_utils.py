...
def test_unpack_mat(temp_files):
    ...
    with h5py.File(temp_mat_file, "w") as f:
        ...
        dataset = group.create_dataset("dataset", data=np.array([1, 2, 3], dtype="float64"))
...
def test_convert_mat_char(temp_files):
    temp_mat_file, _ = temp_files
    with h5py.File(temp_mat_file, "w") as f:
        dataset = f.create_dataset("char_dataset", data=np.array([72, 101, 108, 108, 111], dtype="uint8"))
...
def test_convert_mat_logical(temp_files):
    temp_mat_file, _ = temp_files
    with h5py.File(temp_mat_file, "w") as f:
        dataset = f.create_dataset("logical_dataset", data=np.array([1, 0, 1], dtype="uint8"))
