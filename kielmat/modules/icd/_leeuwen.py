...
class LeeuwenInitialContactDetection:
    """
    The Leeuwen initial contact detection algorithm identifies initial contacts (heel strikes)
    ...
    relative signal is zero-centred and low-pass filtered with a zero-phase Butterworth filter. Initial
    contacts coincide with the most anterior position of the heel, i.e. the positive peaks of the
    relative signal, which are located with prominence- and distance-constrained peak detection.
    ...
    Finally, the initial contacts information is provided as a DataFrame with columns `onset`,
    `duration`, `event_type`, `side`, and `tracking_system`.

    Methods:
        detect(data, sampling_freq_Hz, heel_col_name, reference_col_name, ...):
            Detects initial contacts from the marker position data.

    Examples:
        Find initial contacts of the left leg from heel and pelvis markers
        ...
        >>> print(icd.initial_contacts_)
            ...
            0   1.23    0          initial contact   left   omc
            1   2.34    0          initial contact   left   omc
    ...
    def __init__(
        ...
        """
        Initializes the LeeuwenInitialContactDetection instance.
        """
    ...
    def detect(
    ...
    ) -> "LeeuwenInitialContactDetection":
        """
        Detects initial contacts (heel strikes) based on the input marker position data.
        ...
        Args:
            ...
            min_step_time_s (float, optional): Minimum time between consecutive initial contacts in seconds, used as the minimum peak distance. Default is 0.5.
        ...
        Returns:
            LeeuwenInitialContactDetection: Returns an instance of the class.
                ...
                which is a pandas DataFrame in BIDS-like format with the following columns:
                    - onset: Time of the initial contact (seconds, or datetime if dt_data is provided).
                    - duration: Duration of the event (always 0 for instantaneous contacts).
                    - event_type: Type of the event (always 'initial contact').
        ...
        # Initial contacts are the most anterior heel positions -> positive peaks of the signal.
        min_distance = max(1, int(min_step_time_s * sampling_freq_Hz))
        prominence = np.std(filtered_signal) * prominence_factor
        ic_samples, _ = scipy.signal.find_peaks(
            filtered_signal,
        ...
        onsets = ic_samples / sampling_freq_Hz
        self.initial_contacts_ = pd.DataFrame(
            {
                "onset": onsets,
                "duration": 0,
        ...
        if dt_data is not None:
            self.initial_contacts_["onset"] = dt_data.iloc[ic_samples].reset_index(
                drop=True
            )
