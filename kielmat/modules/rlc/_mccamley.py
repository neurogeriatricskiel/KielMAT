...
class MacCamleyInitialContactClassification:
    ...
    specifically vartical gyro, anterior-posterior gyro, or a combination of both. The sign of the processed signal at the 
    initial contact position is then used to distinguish between left and right ICs.

    Originally, the algorithm relied on the vartical axis signal alone [1]. However, Ullrich et al. [2] demonstrated that 
    ...
    def detect(
        ...
        Args:
            ...
            ic_timestamps (pd.DataFrame): Detected initial contact (IC) events, generated from the Paraschiv-Ionescu Initial Contact Detection algorithm.
            signal_type (str, optional): Signal type for classification ('vertical', 'ap', 'combined'). Default is 'vertical'.
        
        Returns:
            pd.DataFrame: DataFrame with initial contact onset and their corresponding labels as right or left.
        """
        # Extract vertical and anterior-posterior data
        gyro_vertical = gyro_data[v_gyr_col_name].to_numpy()
        gyro_ap = gyro_data[ap_gyr_col_name].to_numpy()

        # Subtract mean from vertical and anterior-posterior signal
        gyro_vertical = gyro_vertical - np.mean(gyro_vertical)
        gyro_ap = gyro_ap - np.mean(gyro_ap)
        
        # Preprocess gyro vertical and anterior-posterior data
        ...
        # Calculate gyro combined as (gyro combined = gyro vertical - gyro anterior-posterior)
        gyro_combined = filtered_gyro_vertical - filtered_gyro_ap

        # Get Innitial Contacts (ICs) indices and convert it to indices using sampling frequency
        ...
        if signal is None:
            raise ValueError("Invalid signal type. Choose 'vertical', 'ap', or 'combined'.")
