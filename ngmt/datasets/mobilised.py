...
def _load_file(file_path: str) -> dict:
    ...
    sub_path, session_name = os.path.split(
        os.path.split(file_path)[0]
    )
    base_path, sub_id = os.path.split(
        sub_path
    )
    ...
    for i, tracked_point in zip(
        range(num_tracked_points),
        indip_data.keys()
    ):
        ...
        for ch_type in indip_data[tracked_point]["Fs"].keys():
            ...
            channels_dict["name"] += [f"{tracked_point}_{_MAP_CHANNEL_NAMES[ch_type]}_{x}" for x in ["x", "y" "z"]]
            ...
            channels_dict["sampling_frequency"] += [indip_data[tracked_point]["Fs"][ch_type] for _ in range(3)]
            ...
            if data is None:
                ...
                data = np.concatenate(
                    (
                        data,
                        indip_data[tracked_point][ch_type]
                    ),
                    axis=1
                )
