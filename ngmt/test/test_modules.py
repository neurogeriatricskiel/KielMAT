...
def test_detect():
    ...
    assert (
        "onset" in gait_sequences.columns
    ), "Gait sequences should have 'onset' column."
    assert (
        "duration" in gait_sequences.columns
    ), "Gait sequences should have 'duration' column."
    assert (
        "event_type" in gait_sequences.columns
    ), "Gait sequences should have 'event_type' column."
    assert (
        "tracking_systems" in gait_sequences.columns
    ), "Gait sequences should have 'tracking_systems' column."
    assert (
        "tracked_points" in gait_sequences.columns
