...
def import_apdm_mobilitylab(
    file_name: str | Path, tracked_points: str | list[str]
    ...
    with h5py.File(file_name, "r") as hfile:
        ...
        if version == 5:
            ...
            sensor_to_label = {
                sensor_id: sensors_group[sensor_id]["Configuration"]
                .attrs["Label 0"]
                .decode("utf-8")
            ...
            tracked_points = [
                sensor
                for sensor, label in sensor_to_label.items()
                if label in tracked_points
            ]
            ...
            if invalid_tracked_points:
                raise ValueError(
                    f"The following tracked points do not exist in sensor names: {invalid_tracked_points}"
                )
